    ...
    <el-dialog width="51%" :title="title" :close-on-click-modal="false" :visible.sync="visible" :destroy-on-close="true" @close="handleDialogClose">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="90px" class="demo-ruleForm" size="mini">
        ...
        <el-form-item label="处理证明" prop="recordFile">
          <div v-if="isShow ==='noSubmitDetail'" class="fileContaier">
            ...
            <a
              v-if="viewFileTypeNew !== 'img' && ruleForm.recordFile && ruleForm.recordFile!=='因超时系统自动确认'"
        ...
        <el-form-item label="处理记录" prop="record">
          <span v-if="isShow ==='noSubmitDetail'">{{ ruleForm.eventRecord || '--' }}</span>
          <el-input
            v-else
        ...
        <el-form-item v-if="isShow ==='noSubmitDetail'" label="处理人" prop="eventHandler">
          <span>{{ ruleForm.eventHandler || '--' }}</span>
        </el-form-item>
        <el-form-item v-if="isShow ==='noSubmitDetail'" label="处理时间" prop="eventHandleTime">
          <span>{{ ruleForm.eventHandleTime || '--' }}</span>
        </el-form-item>
        <div v-if="ruleForm&&ruleForm.fileList&&ruleForm.fileList.length>0" style="margin-left: 148px;margin-top: -20px">
          <div v-for="item in ruleForm.fileList" :key="item.id" class="container_list">
            <a>
              <i class="el-icon-paperclip" />
              <span style="color: #347fff;margin-left:10px">{{ item.filename }}</span>
              <i style="margin-left: 10px" class="el-icon-close show" @click="DeleteFiles(item.id)" />
            </a>
          </div>
        </div>
        <el-form-item v-if="isShow ==='noSubmit'" class="button">
        ...
        <el-form-item v-else class="button">
          <el-button type="primary" class="viewButton" @click="handleForm('ruleForm')">撤销无需处理</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    <el-dialog
      title="提示"
      :visible.sync="isShowSubmitForm"
      width="35%"
    >
      <span>是否确认撤销无需处理？确认后事件返回未确认状态。</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="isShowSubmitForm = false">取 消</el-button>
        <el-button type="primary" @click="handleChangeTs()">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // type 1:人员  0:事件
          formData.set('type', 0)
          console.log('222222fileList', this.fileList)
          if (this.fileList.name) {
          } else {
            const params = {}
            params.states = '无需处理'
            params.ids = this.data.eventId
            params.records = this.ruleForm.record || '无需处理'
          }
        } else {
