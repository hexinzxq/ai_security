          ...
          :class="
            item.status === 1
              ? item.deviceState === 1
                ...
                : 'equipmentOne noise-box-content_red'
              : item.deviceState === 1
                ? 'equipmentOne equipmentOne-grey'
...
export default {
  ...
  methods: {
    noiseRouter() {
      this.$router.push('/env/meteorologicalChart')
    },
    jumpToNoise(id, name, status, message) {
      if (message > 0) {
        status = '1'
        this.$router.push({
          name: 'EnvMeteorological',
          query: {
            id,
            name,
            status
